...
async function saveGuess(matchId){
  ...
  const { count } = await sb.from('guesses').select('*',{count:'exact',head:true}).eq('match_id',matchId).eq('guess_a',a).eq('guess_b',b).neq('player_id',session.user.id);
  if(count>0){
    if(!confirm(`Seu palpite é idêntico ao de outros ${count} participante(s).\n\nCaso este placar seja vencedor, o prêmio será dividido entre ${count+1} jogadores (incluindo você).\n\nPressione OK para confirmar ou Cancelar para alterar.`)) return;
  } else {
    toast('Seu palpite é exclusivo até este momento.');
  }
